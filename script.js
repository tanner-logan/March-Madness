...
document.querySelector(".Submit").addEventListener("click", function () {
	try {
		...
		let currTeam1 = "";
		let currTeam2 = "";
		let teamList = [Houston, SIUEdward, Gonzaga, Georgia, Clemson, McNeese, Purdue, HighPoint, Illinois, Xavier, Kentucky, Troy, UCLA, UtahSt, Tennessee, Wofford, Florida, NorfolkSt, UConn, Oklahoma, Memphis, ColoradoSt, Maryland, GrandCanyon, Missouri, Drake, TexasTech, NCWilmington, Kansas, Arkansas, StJohns, Omaha, Duke, American, MississippiSt, Baylor, Oregon, Liberty, Arizona, Akron, BYU, VCU, Wisconsin, Montana, SaintMarys, Vanderbilt, Alabama, RobertMorris, Auburn, AlabamaSt, Louisville, Creighton, Michigan, UCSD, TexasAM, Yale, Mississippi, NorthCarolina, IowaSt, Lipscomb, Marquette, NewMexico, MichiganSt, Bryant, StFrancisPA, Texas, MtStMarys, SanDiegoSt];
		...
		for (let i = 0; i < teamList.length; i++) {
			...
			totalFouls = totalFouls + Number(teamList[i].foulsPG);
			// GETS THE SEASON TOTAL FREE THROW PERCENTAGES ADDED UP TO LATER GET AN AVERAGE
			totalFTPercentage = totalFTPercentage + Number(teamList[i].ftPercentage);
		...
		const game = {
			...
			injuries: [currTeam1.injuries, currTeam2.injuries],
			foulsPG: [currTeam1.foulsPG, currTeam2.foulsPG],
			assistsPG: [currTeam1.assistsPG, currTeam2.assistsPG],
			pointsPG: [currTeam1.pointsPG, currTeam2.pointsPG],
			oppPointsPG: [currTeam1.opponentsPointsPG, currTeam2.opponentsPointsPG],
		};
